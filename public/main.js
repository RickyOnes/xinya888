    ...
    auth: {
      ...
      signInWithPassword: async (credentials) => {
        try {
          ...
          if (response.access_token) {
            // 优先使用服务器返回的 expires_in 字段来计算 expires_at（单位：秒）
            // 如果没有 expires_in，再使用 expires_at（可能已由 server 计算）
            if (response.expires_in) {
              response.expires_at = Math.floor(Date.now() / 1000) + Number(response.expires_in);
            } else if (!response.expires_at) {
              // 兜底：设置默认 7 天（秒）
              response.expires_at = Math.floor(Date.now() / 1000) + 604800;
            }

            // 确保保存 refresh_token（后续用于续期）
            // response 可能包含 refresh_token 字段
            const toStore = {
      ...
      signUp: async (credentials) => {
        try {
          ...
          // 存储token
          if (response.access_token) {
            // 同 signInWithPassword，优先使用 expires_in
            if (response.expires_in) {
              response.expires_at = Math.floor(Date.now() / 1000) + Number(response.expires_in);
            } else if (!response.expires_at) {
              response.expires_at = Math.floor(Date.now() / 1000) + 604800;
...
async function sendRequest(method, path, data = null) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // 如果是 401，尝试使用 refresh_token 刷新一次 token，然后重试请求一次
      if (response.status === 401) {
        const refreshed = await attemptRefreshAndRetry(url, options);
...
function syncContainersDimensions() {
  ...
  if (tableContainer && chartContainer) {
    ...
    if (returnsTableContainer) {
      returnsTableContainer.style.maxHeight = `${tableHeight}px`;
    }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  registerBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      showRoundedAlert(`注册异常: 该功能已关闭，请联系管理员！`, 'error');
    }
  });
...
async function attemptRefreshAndRetry(url, originalOptions) {
  try {
    ...
    // 使用新的 access_token 重试一次原始请求
    const retryOptions = JSON.parse(JSON.stringify(originalOptions));
    retryOptions.headers = { ...retryOptions.headers, 'Authorization': `Bearer ${toStore.access_token}` };
