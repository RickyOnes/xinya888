...
async function loadSalesData() {
  try {
    ...
    const queryParams = {
      startDate,
      endDate,
      warehouseType: currentWarehouse,
      selectedWarehouses: warehouseMultiSelect.selectedValues,
      selectedBrands: brandMultiSelect.selectedValues,
      selectedProducts: productMultiSelect.selectedValues,
      selectedCustomers: customerMultiSelect ? customerMultiSelect.selectedValues : []
    };
    ...
    const response = await fetch('/api/query-sales', {
      ...
      headers: {
        ...
        'Authorization': localStorage.getItem('supabase.auth.token') 
          ? `Bearer ${JSON.parse(localStorage.getItem('supabase.auth.token')).access_token}`
          : ''
      },
    ...
    showRoundedAlert(`数据加载失败: ${error.message}`, 'error');
    throw error;
  } finally {
    loadingEl.style.display = 'none';
    hideLoadingOverlay();
...
async function load() {
  try {
    const sdata = await loadSalesData();
    console.log('load:',sdata);
    return Promise.resolve();
  } catch (error) {
    return Promise.reject(error);
  }
